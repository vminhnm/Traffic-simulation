...
public abstract class Vehicle implements Movable {
    ...
    private static final double LATERAL_SHIFT_RATE = 28.0;
    private static final double MERGE_BACK_RATE = 22.0;
    private static final double LATERAL_SNAP_EPSILON = 0.25;
    ...
    protected void applyDecision(DrivingDecision decision, double deltaTime) {
        ...
        switch (decision.getAction()) {
            ...
            case MERGE_BACK -> {
                double target = decision.getTargetSpeed() > 0
                        ? decision.getTargetSpeed()
                        : maxSpeed * 0.55;
                currentSpeed = approachSpeed(target, deltaTime);
                stoppedTimer = 0;
                lateralOffset = approachLateralOffset(0.0, MERGE_BACK_RATE, deltaTime);
            }
            case CHANGE_LANE_LEFT -> {
                double target = decision.getTargetSpeed();
                currentSpeed = approachSpeed(target, deltaTime);
                lateralOffset = approachLateralOffset(-SIDE_LANE_OFFSET, LATERAL_SHIFT_RATE, deltaTime);
            }
    ...
    private double approachSpeed(double targetSpeed, double deltaTime) {
        double target = Math.max(0, targetSpeed);
        if (target < currentSpeed) {
            return Math.max(currentSpeed - acceleration * 1.4 * deltaTime, target);
        }
        return Math.min(currentSpeed + acceleration * deltaTime, target);
    }

    private double approachLateralOffset(double targetOffset, double rate, double deltaTime) {
        double clampedTarget = Math.max(-SIDE_LANE_OFFSET, Math.min(SIDE_LANE_OFFSET, targetOffset));
        double diff = clampedTarget - lateralOffset;
        if (Math.abs(diff) <= LATERAL_SNAP_EPSILON) return clampedTarget;

        double remainingRatio = Math.min(1.0, Math.abs(diff) / SIDE_LANE_OFFSET);
        double easedRate = rate * (0.45 + 0.55 * remainingRatio);
        double step = Math.min(Math.abs(diff), easedRate * deltaTime);
        return lateralOffset + Math.signum(diff) * step;
    }
